...
def get_openedx_usage(platform):
    ...
    if request.method == 'GET':
        ...
        if 'till_date' in request.args:
            date_text = str(request.args['till_date'])
            till_date = validate(date_text)
        else:
...
**** API Design
    ...
    - URL :: /analytics/institutes/<platform>
...
def get_openedx_lab_wise_usage(entity):
    ...
    if request.method == 'GET':
        ...
        if "top" in request.args:
            size = validate_top(request.args.get("top"))
        else:
            size = 0
...
** Date Validation
   This function validates the given input is of type =Date=.
#+NAME: validate_date
#+BEGIN_SRC python
def validate(date_text):
    try:
        ...
        message = "Incorrect data format %s provided. Should be YYYY-MM-DD " % date_text
        abort(make_response(jsonify(message=message), 400))

#+END_SRC

** Size Validation
   This function validates the given input is of type =Integer=.
#+NAME: validate_top
#+BEGIN_SRC python
def validate_top(top_value):
    try:
        size= int(top_value)
        return size
    except ValueError:
        message = "Incorrect data format. Top Value should be Integer"
        abort(make_response(jsonify(message=message), 400))
