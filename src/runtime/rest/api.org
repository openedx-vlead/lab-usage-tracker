...
def vlabs_usage(entity):
    ...
    if request.method == 'GET':
        ...
        if entity == "labs":
            ...
            ANALYTICS_DB_API = "%s/%s/openedx" % (APP_URL, entity)
            current_app.logger.debug("Openedx Usage url = %s" % (ANALYTICS_DB_API))
            try:
            ...
            ANALYTICS_DB_API = "%s/%s/nonopenedx" % (APP_URL, entity)
            current_app.logger.debug("Openedx Usage url = %s" % (ANALYTICS_DB_API))
            try:
                res = requests.get(ANALYTICS_DB_API)
                if res.status_code == 200:
                    nonopen_edx_usage = res.json()
                    current_app.logger.debug("%s nonopenedx Usage = %s" % (entity, nonopen_edx_usage))
                else:
                    status_code = res.status_code
                    current_app.logger.error("Error code = %s" % (status_code))
                    abort(status_code)
