...
def get_openedx_usage(entity):
    if request.method == 'GET':
        ...
        if 'till_date' in request.args:
            current_date = request.args['till_date']
        else:
        ...
        if entity == "openedx":
            ...
            ANALYTICS_DB_API = "%s/%s/%s/_count" % (ANALYTICS_DB_URL, index, doc_type)
            ...
            if req.status_code == 200:
                ...
                if len(labs) != 0:
                    ...
                    analytics_data = {"visits" : visits, "hits" : hits, \
                                          "usage" : usage, "till_date" : current_date}
                else:
                    analytics_data = {"visits" : visits, "hits" : hits, \
                                          "usage" : usage, "till_date" : current_date}
            ...
            ANALYTICS_DB_API_FOR_EDX_USAGE = "%s/usage/openedx?till_date=%s" % (APP_URL, current_date)
            ANALYTICS_DB_API_FOR_NONEDX_USAGE = "%s/usage/nonopenedx?till_date=%s" % \
              (APP_URL, current_date)

            try:
                headers = {'Content-Type': 'application/json'}
                r = requests.get(ANALYTICS_DB_API_FOR_EDX_USAGE, \
                                 headers=headers)
                if r.status_code == 200:
                    openedx_usage = r.json()
                    current_app.logger.debug\
                      ("open edx usage = %s" % (r.json()))
                ...
                r = requests.get(ANALYTICS_DB_API_FOR_NONEDX_USAGE, \
                                 headers=headers)
                if r.status_code == 200:
                    nonopenedx_usage = r.json()
                    current_app.logger.debug\
                      ("nonopen edx usage = %s" % (r.json()))
                else:
                    current_app.logger.error\
                ...
                vlabs_usage = {}
                vlabs_usage['openedx_usage'] = openedx_usage
                vlabs_usage['nonopenedx_usage'] = nonopenedx_usage
                vlabs_usage['total_usage'] = openedx_usage['usage'] + nonopenedx_usage['usage']
...
@api.route('/<entity>/nonopenedx', methods=['POST', 'GET'])
...
def nonopenedx_search_usage(entity):
    ...
    if request.method == 'GET':
        ...
        ANALYTICS_DB_API = "%s/%s/%s/_search?q=date:%s&size=1000" % \
          (ANALYTICS_DB_URL, index, type, current_date)
        try:
            req = requests.post(ANALYTICS_DB_API, auth=(USER, PASSWORD), data=json.dumps(query), headers=headers)
            if req.status_code == 200:
                entity_buckets = req.json()['aggregations']['entity']['buckets']
...
** Search usage analytics on non-openedx platfom
   #+TODO
...
def vlabs_usage(entity):
    ...
    if request.method == 'GET':
        ...
        if entity == "labs":
            ...
            abort(500, "Invalid entity")
            
        ANALYTICS_DB_API = "%s/%s/openedx?top=%s" % (APP_URL, entity, size)
        current_app.logger.debug("Openedx Usage url = %s" % (ANALYTICS_DB_API))
        try:
            res = requests.get(ANALYTICS_DB_API)
            if res.status_code == 200:
                open_edx_usage = res.json()
                current_app.logger.debug("%s Openedx Usage = %s" % (entity, open_edx_usage))
            else:
                status_code = res.status_code
                current_app.logger.error("Error code = %s" % (status_code))
                abort(status_code)
        ...
        ANALYTICS_DB_API = "%s/%s/nonopenedx?top=%s" % (APP_URL, entity, size)
        current_app.logger.debug("non openedx Usage url = %s" % (ANALYTICS_DB_API))
        try:
            res = requests.get(ANALYTICS_DB_API)
            if res.status_code == 200:
                non_open_edx_usage = res.json()
                current_app.logger.debug("%s nonopenedx Usage = %s" % (entity, non_open_edx_usage))
            else:
                status_code = res.status_code
                current_app.logger.error("Error code = %s" % (status_code))
                abort(status_code)
        ...
        nol_ids = [entity[entity_id] for entity in non_open_edx_usage ]
        ol_ids = [entity[entity_id] for entity in open_edx_usage ]

        vlabs_ids = list(set(ol_ids) | set(nol_ids))
        vlabs_usage = []

        for vlabs_id in vlabs_ids:
            ol_usage = [ entity for entity in open_edx_usage if entity[entity_id] == vlabs_id ]
            if len(ol_usage) == 0:
                openedx_lab = False
            else:
                openedx_lab = True
            ...
            nol_usage = [ entity for entity in non_open_edx_usage if entity[entity_id] == vlabs_id ]
            if len(nol_usage) == 0:
            ...
            total_usage = vlabs_dict['usage']['openedx_usage'] + vlabs_dict['usage']['nonopenedx_usage']
