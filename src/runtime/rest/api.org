...
@api.route('/openedx/<entity>', methods=['GET'])
def get_openedx_lab_wise_usage(entity):
    ...
    if request.method == 'GET':
        ...
        if entity == "labs":
            ...
            field = "institute_id"
            script = "[doc.institute_id.value, doc.lab_name.value, doc.discipline_name.value, doc.institute_name.value].join('::')"
            
        elif entity == "disciplines":
            field = "discipline_id"
            script = "[doc.discipline_id.value, doc.lab_name.value, doc.discipline_name.value, doc.institute_name.value].join('::')"
        else:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for lab_usage in labs_usage:
                    lab_usage_dict = {}
                    lab_usage_dict[field] = str(lab_usage['key'])
                    lab_details = lab_usage['entity_details']['buckets'][0]['key'].split("::")
