...
def fetch_analytics(analytics):
		...
		es = Elasticsearch(hosts = [elastic_host])
                elastic_index = "vlabs"		
 		if es.indices.exists(elastic_host):
    			current_app.logger.debug("Index already exists")
		else:
#        		request_body = {
#                		"index" : {
#		                  "analysis": {
...
#       		    		}
		        ...
		        current_app.logger.debug("creating '%s' index..." % (elastic_index))
        		res = es.indices.create(index = elastic_index, body = request_body)
        		current_app.logger.debug(" response: '%s'" % (res))

                	es = Elasticsearch([{'host':elastic_host, 'port': elastic_port}])
	                es.index(index = elastic_index, doc_type = usage, body=data_dict)
