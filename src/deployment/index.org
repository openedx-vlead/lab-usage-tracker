...
* Script to get openedx user analytics
  ...
  #+BEGIN_SRC python
    ...
    def connect_db():
        db = MySQLdb.connect("localhost","<user>","<paswd>","edxapp" )
        cursor = db.cursor()

    def dis_connect_db():
        db.close()

    def get_user_count(query):
        try:
    ...
    if __name__== "__main__":
        ...
        active_users = get_user_count(active_users_query)
        inactive_users = get_user_count(inactive_users_query)
        registered_users = get_user_count(registered_users_query)
        ...
        dis_connect_db()
        with open('/root/edx_user_analytics.json', 'w') as f:
